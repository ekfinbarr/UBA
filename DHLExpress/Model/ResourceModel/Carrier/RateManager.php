...
namespace UBA\DHLExpress\Model\ResourceModel\Carrier;

use UBA\DHLExpress\Model\Config\Source\RateConditions;
use UBA\DHLExpress\Model\ResourceModel\Carrier\Rate\CSV\ColumnNotFoundException;
use Magento\Framework\Filesystem;
use Magento\Framework\Filesystem\DirectoryList;
use UBA\DHLExpress\Model\ResourceModel\Carrier\Rate\Import;
use UBA\DHLExpress\Model\ResourceModel\Carrier\Rate\RateQuery;
use UBA\DHLExpress\Model\ResourceModel\Carrier\Rate\RateQueryFactory;
...
class RateManager extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    {
        $this->_init('uba_dhlexpress_rates', 'pk');
    }
    ...
    public function uploadAndImport(\Magento\Framework\DataObject $object)
    {
         ...
         */
        if (empty($files['dhlexpress']['groups']['shipping_methods']['groups'][$method]['fields']['import']['value']['tmp_name'])) {
            return $this;
        }
        $filePath = $files['dhlexpress']['groups']['shipping_methods']['groups'][$method]['fields']['import']['value']['tmp_name'];
    ...
    {
        if ($object->getData('groups/dhlexpress/groups/shipping_methods/groups/' . $method . '/fields/rate_condition/inherit') == '1') {
            $conditionName = (string)$this->coreConfig->getValue($path, 'default');
        } else {
            $conditionName = $object->getData('groups/dhlexpress/groups/shipping_methods/groups/' . $method . '/fields/rate_condition/value');
        }
