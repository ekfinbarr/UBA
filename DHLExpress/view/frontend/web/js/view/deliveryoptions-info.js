define([
    ...
    return Component.extend({
        defaults: {
            template: 'UBA_DHLExpress/deliveryoptions-info'
        },
        ...
        initObservable: function () {
            ...
            window.uba_dhlexpress_services_current_request_sequence = 0;
            window.uba_dhlexpress_services_current_request_sequence_validation = 0;
            window.uba_dhlexpress_services_current_request_timeout = 0;
            ...
            quote.shippingAddress.subscribe(function () {
                ...
                if (country_memory !== quote.shippingAddress().countryId) {
                    ...
                    $.post(urlBuilder.build('dhlparcel_shipping/deliveryservices/available'), data, function (response) {
                        ...
                        window.uba_dhlexpress_deliveryservices_enabled = data.enabled;
                ...
                $.post(urlBuilder.build('dhlparcel_shipping/deliverytimes/times'), times_data, function (response) {
                    ...
                    if (
                        ...
                        window.uba_dhlexpress_deliverytimes_validated = true;
                    }
                }, 'json');
            });
            ...
            $.post(urlBuilder.build('dhlparcel_shipping/deliverytimes/enabled'), {}, function (response) {
                ...
                window.uba_dhlexpress_deliverytimes_enabled = data;
            }, 'json');
            ...
            this.selectedTime.subscribe(function(latest) {
                ...
                $.post(urlBuilder.build('dhlparcel_shipping/deliverytimes/sync'), data, function (response) {
                    ...
                    window.uba_dhlexpress_deliverytimes_validated = data;
                    $('#uba-dhlexpress-deliverytimes-info-error').hide();
                }, 'json');
            }, this);
            ...
            this.selectedServices.subscribe(function(latest) {
                ...
                var sequence = ++window.uba_dhlexpress_services_current_request_sequence;

                window.uba_dhlexpress_services_current_request_timeout = setTimeout(function() {
                    $(document.body).trigger('uba_dhlexpress:delivery_services_sync', [data, sequence]);
                }, 2000);
            });

            $(document.body).on('uba_dhlexpress:delivery_services_sync', function(e, data, sequence) {
                if (sequence !== window.uba_dhlexpress_services_current_request_sequence) {
                ...
                if (sequence === 1 && $('.iosc-place-order-button').length === 0) {
                    ...
                    window.uba_dhlexpress_services_current_request_sequence_validation = sequence;
                    return;
                }
                ...
                $.post(urlBuilder.build('dhlparcel_shipping/deliveryservices/sync'), data, function (response) {
                    ...
                    window.uba_dhlexpress_services_current_request_sequence_validation = data.sequence;
                    $('#uba-dhlexpress-deliveryservices-info-error').hide();
